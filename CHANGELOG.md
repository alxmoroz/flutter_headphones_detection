...
### Changed
- Removed BroadcastReceiver for wired headphones on Android - now using only AudioDeviceCallback to avoid duplicate events

## [1.1.1] - 2025-11-21

### Fixed
- Fixed headphone detection not working after reconnection on Android
- Fixed potential issues with re-registering listeners by ensuring proper cleanup before new registration in `HeadsetEventsHandler` and `AudioRouteDetector`
- Stream now only emits events on actual state changes, not on subscription (prevents stale state issues)

### Changed
- Removed initial state emission from stream on subscription - initial state should be checked synchronously via `isHeadphonesConnected()` method
